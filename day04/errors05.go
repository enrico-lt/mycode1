...
func obiwan1(name string) (string, error) {
	if name == "" {
...
func main() {
	...
	// run the obiwan1 function
	// use of the "_" means "throw out these results"
	_, err := obiwan1(name)

	// without capturing the results of obiwan1()
	// all we can do is error check
	if err != nil {
		fmt.Println("Could not run the obiwan1 function:", err)
		return
	}
}
